...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      float rank = Float.NEGATIVE_INFINITY;

      if(Arrays.asList(sourceNodes).contains(nodeID)){
        rank = (float) -StrictMath.log(sourceNodes.length); // log(1/n) = -log(n)
      }
  ...
  public int run(String[] args) throws Exception {
    ...
    if (!cmdline.hasOption(INPUT) || !cmdline.hasOption(OUTPUT) || !cmdline.hasOption(NUM_NODES)|| !cmdline.hasOption(SOURCE_NODE_FIELD) ) {
    ...
    conf.setStrings(SOURCE_NODE_FIELD, sources.split(","));
