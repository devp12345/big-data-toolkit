...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    private static Integer[] sourceNodes;

    @Override
    public void setup(Context context) {
      String[] sources = context.getConfiguration().getTrimmedStrings(SOURCE_NODE_FIELD);
      int len = sources.length;
      if (len == 0) {
        throw new RuntimeException(SOURCE_NODE_FIELD + " cannot have length 0! ");
      }

      sourceNodes = new Integer[len];

      for(int i  = 0; i < len; ++i){
        sourceNodes[i] = Integer.parseInt(sources[i]);
      }
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        // how much we have to give to each of the source nodes
        // equiv to log(P/m)
        float mass = node.getPageRank() - (float) StrictMath.log(sourceNodes.length);

        for(Integer sourcenode : sourceNodes){
            neighbor.set(sourcenode);
            intermediateMass.setNodeId(sourcenode);
            intermediateMass.setType(PageRankNode.Type.Mass);
            intermediateMass.setPageRank(mass);
...
//   }
  ...
  private static class ReduceClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      // calculate mass from links and jumps
      float link = mass + (float) StrictMath.log(1 - ALPHA); // log(mass * prob_of_following_link) since we might jump
      float jump = Float.NEGATIVE_INFINITY;

      if(Arrays.asList(sourceNodes).contains(nid.get())){
        jump = (float) (Math.log(ALPHA)-StrictMath.log(sourceNodes.length)); // log(1/n) = -log(n)
      }

      mass = sumLogProbs(link, jump);
  ...
  private static final String SOURCE_NODE_FIELD = "sources";
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("sources").hasArg()
        .withDescription("source nodes").create(SOURCE_NODE_FIELD));
    ...
    if (!cmdline.hasOption(BASE) || !cmdline.hasOption(START) ||
        !cmdline.hasOption(END) || !cmdline.hasOption(NUM_NODES) || !cmdline.hasOption(SOURCE_NODE_FIELD)) {
    ...
    String sources = cmdline.getOptionValue(SOURCE_NODE_FIELD);
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
    ...
    // double resultMass = StrictMath.exp(mass);
    // LOG.info("ITERATION " + j + " HAS MASS " + mass);
  ...
  private float phase1(int i, int j, String basePath, int numNodes,
    ...
    job.getConfiguration().setStrings(SOURCE_NODE_FIELD, sources.split(","));
